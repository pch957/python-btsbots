...
from btsbots.DDPClient import DDPClient
...
class MeteorClient(EventEmitter):
    def __init__(self, url, auto_reconnect=True, auto_reconnect_timeout=0.5, debug=False):
        ...
        self.ddp_client.on('connected', self._connected)
    ...
    async def rpc(self, method, params):
        ...
        try:
            self._wait_for_connect()
            result = await self.ddp_client.rpc(method, params)
        except Exception as e:
            print('unexcept rpc error:', e)
        return result
